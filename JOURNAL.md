# MARCO 1: Análise Exploratória de Dados & Tratamento de Dados
...
Reexecutei todo o fluxo de threshold e filtros da etapa de [Análise Exploratória de Dados](#eda-análise-exploratória-de-dados) e atualizei os indicadores de volumetria para prosseguir de forma segura para a Engenharia de Features.
...
> Nota: Um novo tratamento acrescentou mais uma etapa de limpeza em `title` e criou um novo campo, visando otimizar o processo de buscas. Confira os detalhes na Nota de Atualização: [Retorno ao Marco 1](#nota-de-atualização-retorno-ao-marco-1).
...
## Primeiras Explorações e Bloqueios

> **Dúvida:** No início desta etapa, surgiu uma certa dúvida sobre como prosseguir. Como estava lidando com dois conjuntos de dados distintos, comecei a pesquisar quais modelos são comumente aplicados a sistemas de recomendação e como estruturar esse tipo de pipeline do zero, mas não sabia ao certo quais termos buscar. 

> **Dúvida:** Fiquei confuso sobre quais tipos de abordagens de recomendação existem, quais delas se aplicavam ao meu cenário e onde encontrar essas informações. Esse desconhecimento inicial começou a me travar um pouco, gerando a necessidade de buscar materiais teóricos para entender os conceitos de base antes de escrever o código.

> **Dúvida:** Uma das minhas principais dúvidas conceituais era: em Machine Learning supervisionado, o padrão é dividir o conjunto de dados em treino e teste; mas como funciona essa validação em sistemas de recomendação?

Para destravar e alinhar os conceitos fundamentais, consultei os seguintes materiais:
*   [Entendendo Sistemas de Recomendação (Medium)](https://arturlunardi.medium.com/entendendo-sistemas-de-recomenda%C3%A7%C3%A3o-c50a20856394)
*   [O que é um mecanismo de recomendação? (IBM)](https://www.ibm.com/br-pt/think/topics/recommendation-engine)
*   [Recommender Systems: A Complete Guide (Towards Data Science)](https://towardsdatascience.com/recommender-systems-a-complete-guide-to-machine-learning-models-96d3f94ea748/)

Após o estudo, decidi que o projeto final será um sistema híbrido. Para começar, optei por aplicar uma abordagem baseada em conteúdo (*Content-Based Filtering*) focada exclusivamente no dataset de filmes. A intenção é aproveitar o tratamento que realizei na etapa anterior com o *one-hot encoding*, utilizando os gêneros para construir a modelagem inicial.

> **Dúvida:** Ainda tenho dúvidas se devo utilizar todas as *features* disponíveis ou isolar apenas algumas. O campo de gêneros é o mais evidente para a modelagem. O ano também pode ser útil, por mais que represente um intervalo contínuo de dados (e não valores binários de 0 ou 1). Mas e a coluna `title`, valeria a pena utilizá-la?
 

## Content-Based Filtering (Filtragem Baseada em Conteúdo)

> **Problema Encontrado:** Ao testar a busca, me deparei com a inconsistência de artigos deslocados para o final do título (como o padrão ", The"). Para resolver isso, decidi interromper o fluxo, voltar ao notebook `01_eda_and_cleaning.ipynb` e criar uma nova coluna de tratamento para os títulos. Essa mudança de rota está detalhada na subseção [Nota de Atualização - Retorno ao Marco 1](#nota-de-atualização-retorno-ao-marco-1).

Com a pipeline corrigida, carreguei o dataset tratado `movies_cleaned` contendo a nova coluna `search_title`. A partir dele, dividi os dados em dois novos dataframes:
1. `df_meta`: Contendo os metadados de identificação (`movieId`, `search_title`, `title`, `year`).
2. `X_genres`: Composto estritamente pelas colunas de gêneros codificadas em *one-hot encoding*.

A similaridade entre os filmes foi calculada a partir da matriz `X_genres` utilizando o método de similaridade de cosseno. Coletei referências teóricas para justificar matematicamente a escolha dessa métrica matemática [(detalhada na seção abaixo)](#justificativa-da-escolha-similaridade-de-cosseno).

Em seguida, estruturei a função `get_content_recommendations`, que recebe o título de um filme pesquisado e retorna os 5 itens mais similares do catálogo.

Como observação importante, notei que este sistema inicial ainda é muito simples e limitado. Muitos filmes atingem o valor máximo de similaridade (1.0) apenas por compartilharem exatamente os mesmos gêneros. Compreendi que essa abordagem funciona bem como um primeiro filtro de qualidade, mas precisa ser complementada por técnicas mais sofisticadas no futuro (como a inclusão da variável `year` no cálculo de distância ou a junção com métodos colaborativos).

> **Dúvida:** Ainda me sinto confuso sobre como avaliar numericamente os retornos dos experimentos. Como não estamos em um cenário supervisionado clássico (como uma tarefa de classificação), sinto falta de métricas tradicionais como acurácia para validar o modelo.

### Justificativa da Escolha: Similaridade de Cosseno

Para calcular a similaridade entre os filmes com base em seus gêneros, foi utilizado o cálculo de **Similaridade de Cosseno**. Esta é uma das abordagens mais consolidadas e eficientes na literatura de sistemas de recomendação baseados em conteúdo, sendo adequada para vetores esparsos e binários (como a matriz de gêneros codificada em *One-Hot Encoding*).

Matematicamente, a similaridade de cosseno mede o cosseno do ângulo entre dois vetores em um espaço multidimensional. A fórmula geral é definida como:
...
Onde:
*   **O numerador ($A \cdot B$):** Representa o produto escalar dos dois vetores. No cenário de dados binários ($0$ e $1$), ele realiza uma operação lógica equivalente a contar o número de gêneros que ambos os filmes possuem em comum (interseção).
*   **O denominador ($\|A\| \|B\|$):** Representa o produto das normas euclidianas de cada vetor. Ele atua como um fator de normalização baseado na "quantidade total" de gêneros que cada filme possui individualmente.

#### Exemplo Prático de Funcionamento

Para compreender o impacto da normalização na prática, considere os seguintes filmes do catálogo:
*   **Filme A (Casper):** Possui os gêneros *[`Adventure`, `Children`]* $\rightarrow$ vetor: $[1, 1, 0]$
*   **Filme B (Jumanji):** Possui os gêneros *[`Adventure`, `Children`, `Fantasy`]* $\rightarrow$ vetor: $[1, 1, 1]$
...
> Nota de Estudo: Durante as pesquisas sobre métricas de similaridade, encontrei a técnica *TF-IDF (Term Frequency-Inverse Document Frequency)*. De forma bem resumida e inicial, ela serve para dar pesos diferentes aos elementos:
>*   **TF (Frequência do Termo):** Mede o quanto um termo aparece em um único item.
>*   **IDF (Frequência Inversa nos Documentos):** Penaliza termos que aparecem em quase todo o catálogo (gêneros muito comuns, como `Drama`) e dá muito mais valor para termos raros (como `Film-Noir`).
>
>Embora seja uma técnica muito interessante para fazer o sistema entender que gêneros raros deveriam aproximar mais dois filmes do que gêneros comuns, cheguei à conclusão de que aplicá-la aqui seria overengineering.
>
>Como o dataset atual possui uma lista fechada e binária de apenas 19 gêneros (sem sinopses ou textos longos onde o TF-IDF realmente brilha em Processamento de Linguagem Natural), a similaridade de cosseno pura atende perfeitamente ao objetivo. Deixo este registro como um aprendizado e como uma possível melhoria para testar no futuro ou aplicar em datasets mais completos.

Sobre a dúvida de como avaliar o modelo sem as métricas tradicionais de aprendizado supervisionado (como acurácia), a validação qualitativa olhando os resultados diretamente foi suficiente. Como o objetivo aqui não é refinar ou criar o recomendador perfeito, mas sim estruturar um pipeline inicial que funcione, os testes com filmes específicos provaram que a lógica está funcionando.
